...
- [ ] Return thread-contract violations through `Expected`.
...
- [ ] Use one deadline for each gateway operation.
  - Start one deadline when `call()` begins.
  - Share the remaining time between the service-availability wait and the response wait.
  - Update the timeout documentation and tests to cover the total operation time.

- [ ] Explicitly initialize the unbound thread ID.
